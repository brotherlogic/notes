...
- **Synchronize before starting work**: Always check out the `main` branch and synchronize it to the remote HEAD (e.g., `git checkout main && git pull origin main`) before starting any new issue work. This guarantees a clean, up-to-date base and avoids merge conflicts.
- **Issue Workflow**: Always follow the structured requirements gathering and design workflows outlined in the [ISSUES.md](file:///workspaces/tasks/ISSUES.md) specification when collaborating on new issues.
