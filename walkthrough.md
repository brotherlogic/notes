...
- **Automated Pull Request Builder Workflow (`pr-builder.yml`)**: Fires automatically on pushes to any non-main branches. Checks if an active PR already exists for the head branch, and if not, automatically generates a new PR targeted at `main` and assigns it directly to the pushing developer (`github.actor`) for lifecycle ownership. Connects securely using `secrets.PERSONAL_TOKEN` (belonging to automation user `brotherlogic-automation`) and includes protection boundaries to specifically ignore pushes by `brotherlogic-automation` and the standard GHA bot.
